function argout = filtro(archivo,numero,umbral) %archivo: cadena de caracteres con el nombre del archivo/ numero: 7 sensor amarillo, 8 sensor azul/umbral: tiempo en minutos del tiempo entre grupos de medidas
%%La idea es tomar un archivo .csv sacado del fotometro
format long
M = csvread(archivo,1,0); %Ingreso archivo desde fotometro
...
for i=1:(length(M(:,1))-1)
    if ( datenum(M(i,1:6)) + datenum([0 0 0 0 umbral 0]) ) > datenum(M(i+1,1:6)) %Comparamos fecha de la medicion con la siguiente
        j = j + 1; %de cumplirse agregamos el indice al grupo
    else  %si no cerramos el grupo y buscamos el optimo
        yel = find(max(M(k:j,numero))); %optimo sensor amarillo
